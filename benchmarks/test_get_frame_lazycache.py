...
from pycamhd import lazycache as pycamhd
import numpy as np
...
def do_get_frame(benchmark, filename, format):
    ...
    meta=pycamhd.get_metadata(test_lazycache+filename)
    img=benchmark(pycamhd.get_frame, test_lazycache + filename, frame_num=random.uniform(0,meta['NumFrames']), format=format)
